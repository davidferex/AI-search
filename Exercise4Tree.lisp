;;; Code for the exercise 4 of homework 4, TREE search
...
(defun main (city strategy)
  ...
  (time (make-tree-search (make-node :city (get-city-from-list city *all-cities-list*) :cost 0 :depth 0) strategy)))
...
(defun all-cities-from-htable (*all-cities-htable*)
  (let ((result nil))
	...
	do (push (city-name (gethash key *all-cities-htable*)) result))
    result ))
...
(defun make-tree-search (start strategy)
